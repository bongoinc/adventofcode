import argparse
import hashlib
from part1matrix import Part1Matrix
from part2matrix import Part2Matrix
...
def main():
	...
	part1matrix = Part1Matrix(1000, 1000)
	part2matrix = Part2Matrix(1000, 1000)
	...
	for instruction in f:
		...
		if cmd[0] == 'turn':
			...
			if cmd[1] == 'on':
				...
				part2matrix.turn_on(f[0], f[1], t[0], t[1])
			elif cmd[1] == 'off':
				part1matrix.turn_off(f[0], f[1], t[0], t[1])
				part2matrix.turn_off(f[0], f[1], t[0], t[1])

		elif cmd[0] == 'toggle':
			f = get_coord(cmd[1])
			t = get_coord(cmd[3])
			part1matrix.toggle(f[0], f[1], t[0], t[1])
			part2matrix.toggle(f[0], f[1], t[0], t[1])

	print("PART1: There are %d light bulbs lit!" % part1matrix.lit())
	print("PART2: Total amount of brightness is %d!" % part2matrix.sum())
